...
export async function createCategory(data: { name: string, image?: string, is_active?: boolean }) {
    const token = getToken();
...
export async function updateCategory(id: number, data: { name?: string, image?: string, is_active?: boolean }) {
  const token = getToken();
